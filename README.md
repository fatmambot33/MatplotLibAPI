...
The server provides `plot_bubble`/`plot_bubble_from_csv` and `plot_network`/`plot_network_from_csv` tools. Each accepts either a CSV path or a table payload and returns PNG bytes as an octet payload.
